#!/usr/bin/env python3
"""
ULP Combo Extractor Pro — Neon Edition v3
Author: ａᏰᏥì ⃟  🇺🇸|🕯️🍷 (meltingYoureEgo)
GitHub: https://github.com/meltingYoureEgo
Description: Follower-Only combo extractor. Only works for followers.
"""
...
GITHUB_API_URL = f"https://api.github.com/users/{GITHUB_USERNAME}/followers"
VERSION = "3.0"
...
def get_github_username():
    """Get GitHub username from git config or ask user."""
    try:
        import subprocess
...
def process_file(filepath, filter_keyword=None):
    ...
    # Extract combos (email:pass or user:pass)
    extracted = []
    domains = {}
    ...
    for line in lines:
        ...
        # Pattern: something@something.something:password
    ...
    with open(output_file, 'w', encoding='utf-8') as f:
        for combo in sorted(set(extracted)):  # Remove duplicates
            f.write(combo + '\n')
    ...
    if domains:
        ...
        # Ask if user wants to save domain-specific files
...
def main():
    ...
    if not is_follower(username):
        ...
        print(f"👉 Follow @{GITHUB_USERNAME} first, then run this script again.")
        print(f"📎 GitHub profile: https://github.com/{GITHUB_USERNAME}")
        sys.exit(1)
    
    print(f"\n✅ Verified! @{username} is a follower. Welcome, {AUTHOR_NAME} appreciates your support!")
